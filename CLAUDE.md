...
- Release: bump `package.json` → **`bun run atlas:json`** → **`bun run docs:deps`** → commit → push master → tag `v0.X.Y`
 ...
 ⚠ **`bun run atlas:json` belongs AFTER the bump and BEFORE `docs:deps`**, for exactly the
 reason the `DEPENDENCY_GRAPH.md` rule above exists: `data/atlas/oscillators.json` embeds
 `packageVersion`, so regenerating it before the bump commits an artifact claiming the OLD
 version. `tests/atlas/atlas-json.test.ts` deep-equals the committed artifact against the live
 registry, so a release that forgets this step fails that test rather than shipping silently -
 which is the gate working, the same way `docs-fresh` caught the v0.44.3 ordering bug.
...
| `src/bridges/` | 55-bridge catalog (IDs 11–65; BE-55…58 added 2026-07-05 — quantum Hall / Casimir / Unruh / Johnson-Nyquist; BE-59…62 the condensed-matter cluster — AC Josephson / fractional QH / Wiedemann-Franz / BCS gap). `index.ts` is the catalog registry (`BRIDGE_EQUATIONS`); `equations/` holds per-bridge AST modules; closed-form evaluators (`gravitational-lensing.ts`, `perihelion-precession.ts`, `be55…be62-*.ts`) sit at this level. **Catalog ≠ graph:** the 55 catalog bridges project to **41 composition-graph edges** (`CATALOG_GRAPH`). **13 bridges are AST-less** (no `BRIDGE_RHS_BY_ID` entry): BE-51/52/55…65. **17 carry no graph edge**, which is a *different* set: BE-28, 29, 32, 35, 40, 44, 55…65. **BE-51 and BE-52 DO have edges** (`be51Edge`, `be52Edge` in `src/composition/edges/calibration.ts`), so they are in the 13 and not in the 17 — this line previously conflated the two sets and is corrected here against a measurement of the built registries (55 rows, 41 edges, 13 AST-less, 17 edgeless). The graph is sparse — `upt map` finds **23 connected components**: one anchored cluster of 16, two small clusters, and a long tail of **20 isolated bridges** that share no quantity with any other edge. The rank-6 tensor framing is aspirational about connectivity the catalog does not yet have; most cross-cluster "links" are dimensional coincidences (see `upt discover` / `docs/research/`). `descriptor.ts` (`getBridge`/`BRIDGE_DESCRIPTORS`) is the unified facade JOINing the three id-keyed registries (metadata + RHS AST + graph edges) into one per-bridge view, guarded against cross-registry drift. |
...
| `src/atlas/` | **Phase 0 atlas pilot** — typed relations BETWEEN models, as opposed to the bridge catalog's relations between QUANTITIES. `types.ts` (`RelationType`, `EvidenceTag`, `Regime`, `ApproximationBound` with a mandatory machine `horizonHolds`, `AtlasBridge`, `AtlasRejection`), `error-algebra.ts` (`composeBounds`, `IDENTITY_BOUND`, `composeBoundPath` — outer-after-inner; `delta` accumulates as `outer.K*inner.delta + outer.delta`), `regime.ts` (π-groups via `buckinghamPi`, keyed by `PiGroup.formula`; dimensionless inputs are passed IN as zero-dimension variables and emerge as trivial groups — never synthesized separately, which would double-add them), `oscillators/` (nine models, five bridges, one rejection), `diffusion/` (Phase 4: eight models, eight bridges incl. the Langevin + Stokes → Stokes–Einstein hyperedge), `waves/` (Phase 4: seven models, seven bridges incl. the Euler + EOS → sound hyperedge; several end in other families — e.g. at the oscillator family's `model-wave-1d` — rather than redefining a model), `families.ts` (`ATLAS_FAMILIES` — whole-atlas gates iterate this, never one family by name), `witness-{symbolic,numeric,result,specs,artifact}.ts` (Phase 4 runners and the registry behind `data/atlas/witness-results.json`, written only by `bun run atlas:witness-results`), `witnesses/`. **Everything is `@internal` and reachable only via the `universal-physics-tensor/atlas` subpath**; nothing is re-exported from `src/index.ts` before ROADMAP Phase 6. **THE INVARIANT IS: NEVER THE BARREL.** `src/bridges/` and `src/composition/` must not import `src/atlas/index.ts`, which would close a cycle `docs:deps` reports. **Corrected 2026-09-22:** this line previously said "types only, and only from `src/atlas/types.ts`", and THE TREE HAS NEVER DONE THAT. Measured: value imports exist at `bridges/index.ts:40` (`deriveRegimeGroups`), `composition/compose.ts:46-47` (`composition-table`, `conventions`) and `composition/graph-viz.ts:28` (`derive-evidence`); type-only imports from `atlas/types.ts` at `bridges/index.ts:36`, `composition/edge.ts:24`, `graph-viz.ts:24`, `uncertainty.ts:25`. `docs:deps` reports **0 circular dependencies**, so the invariant holds while the rule as written did not. A rule STRICTER than the invariant it protects gets silently violated by working code, and the next reader either believes a false description or "fixes" code that was correct. |
