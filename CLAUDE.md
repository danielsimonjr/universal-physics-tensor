...
| `src/bridges/` | 48-bridge catalog (IDs 11–58; BE-55…58 added 2026-07-05 as established closed-form bridges — quantum Hall / Casimir / Unruh / Johnson-Nyquist). `index.ts` is the catalog registry (`BRIDGE_EQUATIONS`); `equations/` holds per-bridge AST modules; closed-form evaluators (`gravitational-lensing.ts`, `perihelion-precession.ts`, `be55…be58-*.ts`) sit at this level. **Catalog ≠ graph:** the 48 catalog bridges project to **41 composition-graph edges** (`CATALOG_GRAPH`) — the 6 closed-form bridges (BE-51/52/55…58) carry no graph edge, and that graph is sparse — `upt map` finds **23 connected components**: one anchored cluster of 16, two small clusters, and a long tail of **20 isolated bridges** that share no quantity with any other edge. The rank-6 tensor framing is aspirational about connectivity the catalog does not yet have; most cross-cluster "links" are dimensional coincidences (see `upt discover` / `docs/research/`). `descriptor.ts` (`getBridge`/`BRIDGE_DESCRIPTORS`) is the unified facade JOINing the three id-keyed registries (metadata + RHS AST + graph edges) into one per-bridge view, guarded against cross-registry drift. |
...
Status distribution across the 48-bridge catalog: 12 established · 33 speculative · 3 highly-speculative · 0 invalid (re-tallied 2026-07-05 after the BE-55…58 PI-instrument expansion — +4 established, data-confrontable bridges; evidence spine 9 → 12).
