...
- [x] **Step 1: Scaffold the package** — create the four config files mirroring `matrix/`, register `tensor` in root `package.json` workspaces + root `vitest.config.ts`. Run `npm install` from the repo root to link the workspace.

- [x] **Step 2: Write the failing test** — `tensor/tests/Tensor.test.ts`
...
- [x] **Step 4: Write `tensor/src/Tensor.ts`**
...
- [x] **Step 5: Write `tensor/src/index.ts`**
...
- [x] **Step 7: Commit** (in the MathTS repo)
...
- [x] **Step 1: Write the failing test** — `tensor/tests/Tensor.ops.test.ts`
...
- [x] **Step 3: Add the `EinsumSpec` type + four methods to `tensor/src/Tensor.ts`**
...
- [x] **Step 4: Update `tensor/src/index.ts`**
...
- [x] **Step 1: Write `tests/numerical/correctness.test.ts`** — small cases vs hand-computed references, run against the default engine.
...
- [x] **Step 2: Write `tests/numerical/einsum-properties.test.ts`** — algebraic identities on random small tensors, run against the default engine.
...
- [x] **Step 3: Write `tests/dimensional/numerical-form-preservation.test.ts`** — the `numericalForm` field survives `raise()` / `lower()` / alpha-conversion.
...
- [x] **Step 4: Run the new tests + full regression**
...
- [x] **Step 1: Re-export the numerical surface from `src/index.ts`**
...
- [x] **Step 2: Write the failing test** — extend `tests/bridges/public-api-stability.test.ts`
...
- [x] **Step 3: Run test to verify it fails / writes the snapshot**
...
- [x] **Step 4: Add `@public` / `@internal` JSDoc tags**
...
- [x] **Step 1 [M]: Cut the MathTS `tensor` package**
...
- [x] **Step 2 [U]: Finalize the UPT optionalDependency**
...
- [x] **Step 4: Update the project memory**
