...
- **Mathematical Formulation** (canonical SYK / Planckian-dissipation form):

<img src="https://i.upmath.me/svg/%5Crho(T)%20%3D%20%5Crho_0%20%2B%20%5Cfrac%7Bk_B%20T%7D%7B%5Chbar%7D%20%5Ccdot%20%5Cfrac%7B1%7D%7Bn_e%20e%5E2%7D%20%5Ccdot%20%5Calpha_%7B%5Ctext%7BSYK%7D%7D" alt="\rho(T) = \rho_0 + \frac{k_B T}{\hbar} \cdot \frac{1}{n_e e^2} \cdot \alpha_{\text{SYK}}" />
...
> **AST encoding (Tier 5):** [`src/bridges/equations/be-25-orch-or.ts`](../../src/bridges/equations/be-25-orch-or.ts) — **stale under reformulation**: the AST module encodes the dropped Penrose-Hameroff `t_OR` form. A future Tier-5 sweep can either retire it or re-encode the IIT Φ_max form (note: Φ is exponential in system size, so AST encoding may not be tractable beyond ~10 elements).
...
**Tractability**: `formally-divergent` — Φ_max computation is exponential in the number of elements (intractable beyond ~10 elements). Approximate measures (Φ*, Φ^G, geometric Φ) exist for larger systems but each gives different numbers and is not interchangeable with Φ_max.
