...
> The public surface is snapshot-tested in `tests/api/public-surface.test.ts`. Any symbol not in that test's `EXPECTED_RUNTIME_EXPORTS` (160 entries) or `ALL_TYPE_EXPORTS` (90 entries) lists is `@internal` and may change without notice.
