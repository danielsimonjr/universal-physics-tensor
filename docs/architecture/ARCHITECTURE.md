...
| Metric | Value |
|--------|-------|
| Source files | 148 TypeScript files |
...
| `composition/` | 17 | Graph-lite `Quantity`/`BridgeEdge`/`composeEdges` layer (v0.8.0) + centralized quantity nodes, alias dispositions, Phase-D enumerator, uncertainty propagation, the identifiability classifier, the retrodiction harness, the unified `explainQuantity` entry point, the (internal) bridge-analysis triage + linkage-map + link-candidate layer, and the 41-edge graph assembled as `CATALOG_GRAPH` (v0.10–v0.11) |
...
### `composition/` (17 files, v0.8.0 → v0.11)
