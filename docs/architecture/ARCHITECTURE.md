...
| Metric | Value |
|--------|-------|
| Source files | 151 TypeScript files |
...
| `composition/` | 19 | Graph-lite `Quantity`/`BridgeEdge`/`composeEdges` layer (v0.8.0) + centralized quantity nodes, alias dispositions, Phase-D enumerator, uncertainty propagation, the identifiability classifier, the retrodiction harness, the unified `explainQuantity` entry point, the (internal) bridge-analysis triage + linkage-map + link-candidate layer, the 41-edge graph assembled as `CATALOG_GRAPH`, and (v0.12) the internal `UniversalTensor`-backed bridge-prediction + the candidate-vetting discovery loop (v0.10–v0.12) |
...
### `composition/` (19 files, v0.8.0 → v0.12)
