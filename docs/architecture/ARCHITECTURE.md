...
| Metric | Value |
|--------|-------|
| Source files | 140 TypeScript files |
...
| `composition/` | 15 | Graph-lite `Quantity`/`BridgeEdge`/`composeEdges` layer (v0.8.0) + centralized quantity nodes, alias dispositions, Phase-D enumerator, uncertainty propagation, the identifiability classifier, the retrodiction harness, the unified `explainQuantity` entry point, and the 41-edge graph (v0.10–v0.11) |
