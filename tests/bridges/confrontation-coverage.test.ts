...
describe('auditCoverage — catalog grounding profile', () => {
  ...
  it('identifies exactly the eight data-confronted bridges (BE-21, BE-23, BE-35, BE-36, BE-37, BE-48, BE-51, BE-52)', () => {
    const confronted = report.bridges
      .filter((b) => b.hasDataConfrontation)
      .map((b) => b.id);
    expect(confronted).toEqual([21, 23, 35, 36, 37, 48, 51, 52]);
    expect(report.withoutDataConfrontation).toBe(36);
    for (const id of [21, 23, 35, 36, 37, 48, 51, 52]) {
  ...
  it('partitions every bridge into one tier; the tiers sum to 44', () => {
    ...
    expect(sum).toBe(44);
    expect(report.byTier['data-confronted']).toBe(8);
  });
