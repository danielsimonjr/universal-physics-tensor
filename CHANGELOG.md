...
- Regenerated `docs/architecture/DEPENDENCY_GRAPH.md` + `dependency-graph.{json,yaml}` with the C-9-fixed vendored generator (commit `9fd4ae9`). The stale "C-9 unfixed" banner is gone; module/file/export stats unchanged — diff is purely the comment-leak fix.

### Fixed

- **BE-33 Hertz-Millis finite-temperature correlation-length exponent corrected: `−ν/z` → `−1/z`.** The encoded scaling `ξ_quantum(T) ~ ξ₀(T/T₀)^(−ν/z)` conflated the two axes of quantum-critical scaling. At a quantum critical point the *temperature* dependence of the spatial correlation length is set by the dynamic exponent `z` alone — `ξ ~ T^(−1/z)` — because temperature fixes the thermal length `L_T ~ T^(−1/z)`; the correlation-length exponent `ν` governs the separate *T=0 tuning-parameter* divergence `ξ ~ |g−g_c|^(−ν)` and cancels out of the scaling function at the critical coupling. Confirmed by a literature check against the QCP-scaling reviews (`cond-mat/0503298` states `ξ ~ (T/Tc)^(−1/z)` explicitly; Hertz 1976; Millis 1993) — this resolves the honest-claude flag in the prior BE-33 `notes` that the `−ν/z` vs `−1/z` convention had not been WebFetch-confirmed. Changed: the AST exponent pin (`−0.71` → `−1`), `evaluateHertzMillis` (now computes `−1/z`; the `nu` input is retained for API stability and is still validated, but no longer affects the result — a future breaking cleanup may remove it), `formula_latex`, the Millis-1993 reference annotation, and `tests/bridges/be-33-encoding.test.ts` (the universality-class-dependent bracket test replaced with a `z`-dependence test and a `ν`-independence regression guard). The dimensional-signature round-trip is unaffected (a dimensionless ratio raised to any exponent stays dimensionless). A second `known_issue` was added flagging that the entry's "Hertz-Millis / 3D Heisenberg, z=1, ν≈0.71" universality-class label is itself questionable — canonical *itinerant* Hertz-Millis sits above its upper critical dimension with mean-field exponents (z=2 AFM / z=3 FM). Source: bridge physics audit, `docs/architecture/BRIDGE-PHYSICS-AUDIT.md` §2/§4. (commit `394d164`)
- **C-9 dep-graph generator bug** (in-tree fix via the vendored `tools/create-dependency-graph/`, commit `384db01`). Source-comment text inside multi-line `export { ... } from` blocks was leaking into re-export symbol rows of `DEPENDENCY_GRAPH.md`; added `stripBraceBlockComments` + `splitBraceSymbols` so comments are stripped while `type X` re-exports are preserved. Closes the "From v0.6.0 doc-integrity review" deferred item — was previously an *external* memoryjs-tool bug, now resolved in-tree.
...
- **Todo bookkeeping** (commit `9f396c1`): v0.6.0 "Latest shipped" clarified tag commit (`ac0cf06`) vs advanced master HEAD; the doc-integrity review and BE-33 fix logged as completed `[x]` items; new "From v0.6.0 doc-integrity review" deferred subsection added (C-9 dep-graph bug; BRIDGE-PHYSICS-AUDIT.md per-bridge follow-ups — ~19 contested framings + questionable `dimensional_signature` tags, not yet triaged).

### Still pending

- **v0.6.0 npm publish remains BLOCKED** on expired `NPM_TOKEN`. Registry still shows `0.5.1`; tag `v0.6.0` at `ac0cf06` publishes cleanly once the token is rotated. User action — see `todo.md`.
- **GitHub release notes for v0.6.0** — to be drafted from the `[0.6.0]` section once npm publish lands (or independently).
