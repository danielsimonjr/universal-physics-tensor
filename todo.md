...
- [ ] **Atlas Sprint 6 — study, scoped release, discovery hypothesis (IN FLIGHT).** **S6.1** — study
      ...
      **S6.7** — `docs/planning/Atlas-API-Review.md`: recommendation only; decision with Mothership (ADR-level).
      ...
      - [ ] Census finding F1: BE-35's encoded residual `R_cross = C²·[g_block(u,v) − g_block(v,u)]` (`src/bridges/equations/be-35-conformal-bootstrap.ts`) omits the `v^d` / `u^d` prefactors of the crossing relation (Rattazzi et al. 2008, eq. 4.3), and crossing holds for the SUM over exchanged operators, not for one block. Decide: re-encode, or state in `known_issues` that the residual is a schematic stub.
