...
- [~] **God-file splits — IN PROGRESS (user chose execute-by-domain 2026-06-22).**
  ...
  - [ ] `quantities.ts` (131 literals) — **split is VIABLE & PLANNED** (measured: 103/105 catalog-consumed quantities are single-domain, only 2 shared → clean domain ownership). PLAN: classify each quantity by its consuming edge's domain across ALL consumers (the 4 new catalog files + `calibration.ts` + `catalog-tranche.ts` + canonical), emit `quantities/{quantum,gravitation-cosmology,fields,condensed-matter}.ts` + `quantities-common.ts` for the ~2 shared + the ~26 used only outside the 4 catalog-domain files, with `quantities.ts` as a barrel re-export. Same parse/brace-match script pattern as the catalog split. **Deferred to a fresh session** (core-registry scripting; avoid context-fatigue error). 
