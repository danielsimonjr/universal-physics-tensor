...
- [ ] **Bridges assessed but NOT re-encoded in v0.6.0 — DESIGN-NOTED 2026-05-23** at `docs/architecture/v0.7-be-x-reencoding-design-note.md`. Per-bridge AST primitive proposals (4 new node kinds), recommended implementation order (BE-13 → BE-39 → BE-50 → BE-19), per-bridge LOC estimates and dependency analysis. Each lands in its own follow-up session; no cross-dependencies between the four. Per-bridge status:
  - **BE-13 Einstein-trace** — needs `TensorTraceNode` (~60 LOC primitive + ~30 LOC re-encoding); recommended FIRST (smallest scope, builds on existing EinsteinFieldEquationNode infra).
  - **BE-19 quantum-bounce** — needs `FriedmannEquationNode` with variant discriminator (~100 LOC + ~40 LOC); recommended LAST (largest variant space).
  - **BE-39 asymptotic-safety** — needs `BetaFunctionNode` + `RGCouplingNode` (~150 LOC + ~80 LOC); recommended SECOND (highest leverage for future RG-related BE entries).
  - **BE-50 Wheeler-Feynman** — needs `GaugeFieldNode` + `TimeSymmetryPredicateNode` (~120 LOC + ~50 LOC); recommended THIRD.
  Phase 1 of the TensorEquationNode generalization (`KleinGordonEquationNode`) shipped same day as the design note, demonstrating the field-equation-helpers pattern; the four BE-X nodes will follow that same shape.
