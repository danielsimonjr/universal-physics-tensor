...
- [ ] **Fix `typed-function` EOVERRIDE blocker (upstream)** — surfaced 2026-05-25 during the v0.7.1 dev-dep validation. `~/Dropbox/Github/typed-function`'s `package.json` has `overrides: { "@rollup/plugin-terser": "^0.4.4" }` that conflicts with a direct dependency on the same package, causing `npm install` to fail with EOVERRIDE. This cascades through the MathTS optional-peer chain: `mathts-core` depends on `typed-function` from the GitHub URL, `mathts-tensor` and `mathts-autograd` depend on `mathts-core`. Net effect: **every `npm install universal-physics-tensor` silently skips the `@danielsimonjr/*` optional peers**. UPT itself works fine (peers gated by runtime presence-checks), but the P8 horizon item ("real-AD test enablement") is blocked until this is fixed. **Resolution scope**: edit `~/Dropbox/Github/typed-function/package.json` to resolve the override conflict, publish fixed typed-function, then `npm install` in UPT should bring in the MathTS chain. Possibly also republish `@danielsimonjr/mathts-core` if its lockfile pinned a broken typed-function.
