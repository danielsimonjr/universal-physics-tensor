...
| `criterion3-study/` | Criterion 3 after the export: `labels.ts` copies the two blind labelers' files into `docs/research/criterion3/labels/` byte for byte and derives the PRIMARY and SECONDARY truth sets (`truth.json`). Run via `bun run atlas:c3-labels -- --labels <dir>`. Zero npm deps. |
